...
export async function runPushRules(context) {
    ...
    const rnDependencyChange = await ensureReactNativePushDependencies(context.rootPath, rnPushVersion, firebaseVersion);
    ...
    if (typeof autoAsk === "boolean") {
        const metaChange = await ensureManifestFlagMeta(manifestPath, "SMT_IS_AUTO_ASK_NOTIFICATION_PERMISSION", autoAsk ? "1" : "0");
        if (metaChange)
...
async function ensurePushDependency(rootPath) {
    ...
    if (originalContent.includes("com.netcore.android:smartech-push")) {
        newContent = originalContent.replace(/api\s*(\(|\s+)['\"]com\.netcore\.android:smartech-push:[^'\")]+['\"]\)?/, depLine);
    }
...
async function ensureReactNativePushDependencies(rootPath, rnPushVersion, firebaseVersion) {
    ...
    if (!nextDependencies[FIREBASE_APP])
        nextDependencies[FIREBASE_APP] = firebaseVersion;
    if (!nextDependencies[FIREBASE_MESSAGING])
        nextDependencies[FIREBASE_MESSAGING] = firebaseVersion;
...
async function ensureAppPushLogic(filePath) {
    const originalContent = await fs.readFile(filePath, "utf-8");
    let newContent = originalContent;
...
async function ensureIndexPushLogic(filePath) {
    const originalContent = await fs.readFile(filePath, "utf-8");
    let newContent = originalContent;
...
function buildUseEffectBlock(needsToken, needsDeeplink, needsForeground) {
