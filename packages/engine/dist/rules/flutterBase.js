...
export async function runFlutterBaseRules(context) {
    const changes = [];
    ...
    if (!appClass) {
        ...
        changes.push(buildChange({
            ...
            originalContent: "",
            newContent,
            summary: "No Application/FlutterApplication subclass detected. Create one, add Smartech init, and register it in AndroidManifest.xml.",
            confidence: 0.3
        }));
...
async function ensureAndroidDependency(rootPath) {
...
async function ensurePubspecDependency(rootPath, version) {
    ...
    if (/smartech_base\s*:/.test(originalContent)) {
        const newContent = originalContent.replace(/smartech_base\s*:\s*[^\n]+/, `smartech_base: ${version}`);
        if (newContent === originalContent)
            return null;
        return buildChange({
...
function getMissingJavaInitLines(source) {
    ...
    if (!hasDebug)
        lines.push(INIT_LINES_JAVA[1], INIT_LINES_JAVA[2]);
    if (!hasTrack)
        lines.push(INIT_LINES_JAVA[3], INIT_LINES_JAVA[4]);
    if (!hasPlugin)
...
function getMissingKotlinInitLines(source) {
    ...
    const lines = [];
    if (!hasInit)
        lines.push(INIT_LINES_KOTLIN[0]);
    if (!hasDebug)
        lines.push(INIT_LINES_KOTLIN[1], INIT_LINES_KOTLIN[2]);
    if (!hasTrack)
        lines.push(INIT_LINES_KOTLIN[3], INIT_LINES_KOTLIN[4]);
    if (!hasPlugin)
        lines.push(INIT_LINES_KOTLIN[5]);
    return dedupeLines(lines);
}
...
function getMissingJavaDeeplinkLines(source) {
    ...
    const lines = [];
    if (!hasVar)
        lines.push(DEEPLINK_LINES_JAVA[0]);
    if (!hasIf)
        lines.push(...DEEPLINK_LINES_JAVA.slice(1));
    return dedupeLines(lines);
}
function getMissingKotlinDeeplinkLines(source) {
    const hasVar = /isDeepLinkFromSmartech\s*\(/.test(source);
    const hasIf = /if\s*\(!isSmartechHandledDeeplink\)/.test(source);
    const lines = [];
    if (!hasVar)
        lines.push(DEEPLINK_LINES_KOTLIN[0]);
    if (!hasIf)
        lines.push(...DEEPLINK_LINES_KOTLIN.slice(1));
    return dedupeLines(lines);
}
async function findLauncherActivity(manifestPath, manifestPackage, sourceRoots) {
    if (!(await pathExists(manifestPath)))
        return null;
