...
const PX_DEP_KTS = "implementation(\"com.netcore.android:smartech-nudges:\" + project.property(\"SMARTECH_PX_SDK_VERSION\"))";
...
async function ensurePxDependency(rootPath) {
    ...
    if (originalContent.includes("com.netcore.android:smartech-nudges")) {
        newContent = originalContent
            ...
            .replace(/implementation\s*(\(|\s+)['\"]com\.netcore\.android:smartech-nudges:[^'\")]+['\"]\)?/, depLine);
    }
...
async function ensureReactNativePxDependency(rootPath, rnPxVersion) {
...
async function ensureHanselMeta(manifestPath, appId, appKey) {
    ...
    if (appId) {
        ...
        if (newContent.includes("HANSEL_APP_ID")) {
            newContent = newContent.replace(/<meta-data[^>]*android:name=\"HANSEL_APP_ID\"[^>]*android:value=\"[^\"]*\"[^>]*\/>/, metaId);
        }
    ...
    if (appKey) {
        ...
        if (newContent.includes("HANSEL_APP_KEY")) {
            newContent = newContent.replace(/<meta-data[^>]*android:name=\"HANSEL_APP_KEY\"[^>]*android:value=\"[^\"]*\"[^>]*\/>/, metaKey);
        }
        else if (/<application[^>]*>/.test(newContent)) {
            newContent = newContent.replace(/<application[^>]*>/, (match) => `${match}\n${metaKey}`);
        }
...
async function ensurePxUseEffect(filePath) {
    const originalContent = await fs.readFile(filePath, "utf-8");
    let newContent = originalContent;
    ...
    const block = buildPxUseEffectBlock(needsInternalListener, needsDeepLinkListener, needsTracker, needsDeeplink);
...
async function findAppEntryFile(rootPath) {
    const candidates = [
...
async function findLauncherActivity(manifestPath) {
    if (!(await pathExists(manifestPath)))
        return null;
...
async function findMainActivityFile(rootPath, manifestName) {
...
async function findByClassName(root, className) {
...
async function findIndexFile(rootPath) {
...
function ensureReactUseEffectImport(source) {
    if (source.includes("useEffect"))
        return source;
    if (source.includes("from 'react'")) {
        return source.replace(/import\s+React\s*(,\s*\{[^}]*\})?\s*from\s+'react';/, (match) => {
            if (match.includes("{")) {
...
function buildPxUseEffectBlock(needsInternalListener, needsDeepLinkListener, needsTracker, needsDeeplink) {
