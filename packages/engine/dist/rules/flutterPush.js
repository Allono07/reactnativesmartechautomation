...
const PUSH_DEP_KTS = "api(\"com.netcore.android:smartech-push:\" + project.property(\"SMARTECH_PUSH_SDK_VERSION\"))";
...
`;
export async function runFlutterPushRules(context) {
    const changes = [];
    ...
    if (typeof autoAskPermission === "boolean") {
        const metaChange = await ensureManifestFlagMeta(manifestPath, "SMT_IS_AUTO_ASK_NOTIFICATION_PERMISSION", autoAskPermission ? "1" : "0");
        if (metaChange)
...
async function ensureGradleProperty(rootPath, version) {
    ...
    if (/SMARTECH_PUSH_SDK_VERSION\s*=/.test(originalContent)) {
        newContent = originalContent.replace(/SMARTECH_PUSH_SDK_VERSION\s*=\s*[^\n]+/, `SMARTECH_PUSH_SDK_VERSION=${version}`);
    }
    else {
...
async function ensurePushDependency(rootPath) {
    ...
    if (originalContent.includes("com.netcore.android:smartech-push")) {
        newContent = originalContent
            ...
            .replace(/(api|implementation)\s*(\(|\s+)['\"]com\.netcore\.android:smartech-push:[^'\")]+['\"]\)?/, depLine);
    }
...
async function ensurePubspecDependency(rootPath, version) {
    ...
    if (/smartech_push\s*:/.test(originalContent)) {
        const newContent = originalContent.replace(/smartech_push\s*:\s*[^\n]+/, `smartech_push: ${version}`);
        if (newContent === originalContent)
            return null;
        return buildChange({
...
async function ensureMainDartPush(filePath) {
    if (!(await pathExists(filePath))) {
        return buildChange({
    ...
    if (stateChange.updated) {
        ...
        const needsHooks = !originalContent.includes("_registerPushToken") ||
        ...
        if (needsHooks) {
            return buildChange({
                ...
                originalContent,
                newContent: originalContent,
                summary: "Could not safely inject push hooks into main.dart. Please add _registerPushToken, foreground handler, and deeplink callback manually.",
                confidence: 0.2,
    ...
    if (newContent === originalContent) {
        const missingSignals = !originalContent.includes("_registerPushToken") ||
            !originalContent.includes("FirebaseMessaging.onMessage") ||
            !originalContent.includes("onHandleDeeplink");
        if (missingSignals) {
            return buildChange({
                ...
                originalContent,
                newContent: originalContent,
                summary: "Could not locate a StatefulWidget State class to inject initState push setup. Add push hooks manually.",
                confidence: 0.2,
                manualSnippet: FLUTTER_PUSH_MANUAL_SNIPPET
            });
        }
...
function ensureBackgroundHandler(source) {
    if (source.includes("firebaseMessagingBackgroundHandler"))
        return source;
...
function ensureMainRegistration(source) {
    ...
    if (braceIndex === -1)
        return source;
    if (source.includes("FirebaseMessaging.onBackgroundMessage"))
        return source;
...
function ensureStatefulPushSetup(source) {
    ...
    const hasRegisterMethodDef = /Future<\s*void\s*>\s+_registerPushToken\s*\(/.test(body) ||
    ...
    if (!hasOnMessage) {
        initLines.push("    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {", "      bool isFromSmt =", "          await SmartechPush().isNotificationFromSmartech(message.data.toString());", "", "      if (isFromSmt) {", "        SmartechPush().handlePushNotification(message.data.toString());", "        return;", "      }", "", "      // Handle non-Smartech notification", "    });");
    }
    if (!hasDeeplink) {
