...
export async function runFlutterPxRules(context: FlutterPxContext): Promise<Change[]> {
  ...
  if (!shouldCreateListeners && (await pathExists(mainDartPath))) {
    ...
    const hasRegistrations =
      /registerPxDeeplinkListener/.test(mainContent) &&
...
async function ensureHanselMeta(
  ...
  if (!hanselAppId || !hanselAppKey) return null;
...
function ensureManifestMeta(content: string, name: string, value: string): string {
...
async function ensureMainDartPx(filePath: string, includeListenerImport: boolean): Promise<Change | null> {
  ...
  if (newContent === originalContent) {
    const missingSignals =
      !originalContent.includes("SmartechPxWidget") ||
...
async function findPxListeners(libDir: string): Promise<{ hasAny: boolean }> {
  if (!(await pathExists(libDir))) return { hasAny: false };
...
function buildPxListenerFileChange(rootPath: string): Change {
...
function buildPxListenerAppendChange(filePath: string, originalContent: string): Change {
...
function wrapAppWithPxWidget(source: string): { updated: boolean; content: string } {
  if (source.includes("SmartechPxWidget(")) return { updated: false, content: source };
...
function ensurePxNavigatorObserver(source: string): { updated: boolean; content: string } {
  if (source.includes("PxNavigationObserver")) return { updated: false, content: source };
  ...
  if (!appMatch || appMatch.index === undefined) return { updated: false, content: source };
...
function ensurePxRegistrations(
  source: string,
  shouldRegister: boolean
): { updated: boolean; content: string } {
  if (!shouldRegister) return { updated: false, content: source };
  let updated = source;
  ...
  if (hasDeeplinkReg && hasInternalReg) return { updated: false, content: source };

  const mainMatch = updated.match(/(Future<void>\s+main\(\)\s*(async\s*)?\{|void\s+main\(\)\s*(async\s*)?\{)/);
  if (!mainMatch || mainMatch.index === undefined) return { updated: false, content: source };

  const braceIndex = updated.indexOf("{", mainMatch.index);
  if (braceIndex === -1) return { updated: false, content: source };
  ...
  if (lines.length === 0) return { updated: false, content: source };
...
async function ensurePxIntentFilter(
  ...
  if (dataMatch) {
    ...
    newContent = originalContent.replace(dataTagRegex, (match) =>
      match.replace(/android:scheme=\"[^\"]+\"/, `android:scheme=\"${scheme}\"`)
    );
  }
...
function resolveActivityClass(name: string, manifest: string): string {
